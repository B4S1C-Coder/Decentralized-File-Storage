...
RUN apt-get update && apt-get install -y \
  ...
  libtool libssl-dev libsodium-dev python3 python3-pip nodejs npm clang zlib1g-dev \
  && apt-get clean && rm -rf /var/lib/apt/lists/*

# Install gRPC v1.66.0 from source
WORKDIR /opt
RUN git clone -b v1.66.0 https://github.com/grpc/grpc.git && \
  cd grpc && git submodule update --init --recursive && mkdir -p cmake/build && \
